...
rule two_read_bam_combiner:
    ...
    shell:
        """
        /usr/bin/time -v samtools faidx {input.REF} 2>> {log}
        /usr/bin/time -v sh -c 'perl scripts/two_read_bam_combiner.pl {input.bam1_filt} {input.bam2_filt} samtools {params.MAPQ_FILTER} | samtools view -bS -t {input.REF}.fai - | samtools sort -@ {threads} -o {output.tmp_bam} - ' 2>> {log}
        """
