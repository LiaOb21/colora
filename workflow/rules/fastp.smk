...
rule fastp:
    ...
    shell:
        """
        /usr/bin/time -v fastp -p -i {input.forward_in} -I {input.reverse_in} -o {output.forward_out} -O {output.reverse_out} --cut_front --cut_front_window_size 5  --detect_adapter_for_pe --json {output.json} --html {output.html} --thread {threads} >> {log} 2>&1
        """
